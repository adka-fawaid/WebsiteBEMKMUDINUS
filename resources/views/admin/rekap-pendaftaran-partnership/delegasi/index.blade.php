...
<div class="overflow-x-auto">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold text-gray-900">Rekap Pendaftaran Delegasi</h3>
    </div>
    ...
    <table class="w-full table-fixed">
        <thead>
            <tr class="bg-gradient-to-r from-gray-50 to-blue-50 border-b-2 border-gray-200">
                ...
                <th scope="col" class="w-[15%] px-4 py-4 text-center">
                    <span class="text-xs font-extrabold text-gray-700 uppercase tracking-wider">Surat Undangan</span>
                </th>
                <th scope="col" class="w-[12%] px-4 py-4 text-center">
        ...
        <tbody>
            @forelse ($rekapDelegasi as $delegasi)
                <tr
                    ...
                    <td class="px-4 py-4">
                        <div class="flex items-center gap-2">
                            <span
                                ...
                                {{ $delegasi->nomor_pendaftaran }}
                            </span>
                        </div>
                    </td>
                    <td class="px-4 py-4">
                        <p class="text-sm font-medium text-gray-900 leading-tight truncate"
                            title="{{ $delegasi->nama_instansi }}">
                            {{ $delegasi->nama_instansi }}
                        </p>
                    </td>
                    <td class="px-4 py-4">
                        <p class="text-sm font-medium text-gray-900 leading-tight truncate"
                            title="{{ $delegasi->nomor_telepon }}">
                            {{ $delegasi->nomor_telepon }}
                        </p>
                    </td>
                    <td class="px-4 py-4">
                        <p class="text-sm font-medium text-gray-900 leading-tight truncate"
                            title="{{ $delegasi->nama_acara }}">
                            {{ $delegasi->nama_acara }}
                        </p>
                    </td>
                    ...
                    <td class="px-4 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button type="button" data-modal-target="show-delegasi-{{ $delegasi->id }}"
                                data-modal-toggle="show-delegasi-{{ $delegasi->id }}"
                ...
                <!-- Include Show Modal -->
                @include('admin.rekap-pendaftaran-partnership.delegasi.show', [
                    'delegasi' => $delegasi,
                ])
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-sm text-gray-500">Belum ada data pendaftaran dari delegasi.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
...
<script>
    // Modal handler for Delegasi tab
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('[data-modal-toggle^="show-delegasi-"]').forEach(function(btn) {
        ...
        document.querySelectorAll('[data-modal-hide^="show-delegasi-"]').forEach(function(btn) {
            btn.addEventListener('click', function() {
